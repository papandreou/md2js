...
module.exports = function (mdSrc, fileName) {
    ...
    var indexOfFreshExpectCodeBlock = 0;
    codeBlocks.forEach(function (codeBlock, i) {
        ...
        var codeBlocksToRun = [];
        if (codeBlock.freshExpect) {
            indexOfFreshExpectCodeBlock = i;
        }

        var cursor = itExpressionStatement.expression.arguments[1].body.body;

        for (var j = indexOfFreshExpectCodeBlock ; j <= i ; j += 1) {
            ...
            if (j === i) {
                ...
                if (typeof codeBlock.output === 'string') {
                    ...
                    check[0].consequent.body[0].argument.arguments[0].value = 'expected example ' + previousExampleNumber + ' not to fail';
                }
