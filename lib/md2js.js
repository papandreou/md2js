...
module.exports = function (mdSrc, fileName) {
    ...
    codeBlocks.forEach(function (codeBlock, i) {
        ...
        itExpressionStatement.expression.arguments[0].value = 'example #' + exampleNumber + ' should ' + (typeof codeBlock.output === 'string' ? 'fail with the correct error message' : 'succeed');
        ...
        for (var j = 0 ; j <= i ; j += 1) {
            ...
            var topLevelStatements = esprima.parse(new Array(codeBlock.lineNumber).join('\n') + '(function () {\n' + codeBlock.code + '}());', { loc: true, source: fileName }).body[0].expression.callee.body.body;
